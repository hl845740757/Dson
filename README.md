...
以下文本为手写文本，用到的特殊标签包括 多行纯文本`ss`、 单行纯文本`sL`。  
...
5. 使用双引号时，换行需要显式使用`\n`。
6. 在纯文本中，内容无需加引号，`\` 不是转义字符，所见即所得，遇见 `~` 行首时表示结束。
7. 在纯文本中，@ss标签未立即换行的情况下，**只有第一空格是分隔字符** —— 立即换行和输入一个空格再换行是等同的。
8. 在纯文本中，**换行后只有行首后的第一个空格是缩进**；双引号字符串换行不可缩进。
9. 单行纯文本与多行纯文本的规则一致，只是单行结束。
10. 存在一些特殊含义的字符串，用户最好总是使用双引号强调是字符串，否则可能发生兼容性问题。

#### 不安全字符

1. 空白字符
2. dson token字符集：花括号`{}`、方括号`[]`、逗号`,` 、冒号`:`、艾特`@`、斜杆 `/` 、反斜杠` \ `
3. 保留字符集：圆括号 `()`
...
header中值得存储的信息：对象自身类型，字典的Key/Value类型，数组的Value类型，数组的长度...
