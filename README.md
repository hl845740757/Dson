...
```
   ...
   --     chengdu
   --   ],
   --   intro: @ss   我是wjybxx，是一个游戏开发者，Dson是我设计的文档型数据表
   -| 达法，你可以通过github联系到我。
   --   thanks
...
| --  | append line | 普通行，上一行换行符有效                                   | -- 这是新的一行      |
| -\| | append      | 合并行，上一行换行符无效                                   | -\| 文本太长了，换行继续 |
| -^  | switch mode | 模式切换，当前行有效；<br/>纯文本模式下插入转义行；<br/>双引号模式下插入纯文本行； |                |
| ~~  | end of text | 表示上一个纯文本输入结束                                   |                |

1. 换行的处理只在**双引号字符串**和**纯文本模式**下有效，其它输入没有换行逻辑。
2. '-^' 行首默认是append模式，即不会插入换行符。
...
| b   | bool      | 5  | bool值                 |                                                                                      | @b true <br> true                                                                                             |
|     | string    | 6  | 字符串                   |                                                                                      | "10"   <br>  abc                                                                                              |
...
| 标签 | 类型                         | 含义                                                                                              | 格式或示例                                                                                                           |
|----|----------------------------|-------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| ss | string string <br/> (text) | 纯文本 <br> 1. 不会对字符进行转义，所见即所得 <br/>2. 遇见普通行(--)时结束 <br/>3. 既可修饰value，**也可修饰key** <br/>4. 相当于特殊的引号 | -- {<br> -- url:  @ss https://github.com/hl845740757 <br> ~~ , reg: @ss ^\[\u4e00-\u9fa5_a-zA-Z0-9]+$ <br> ~~ } |
...
内声明其类型，且声明时和外层括号之间无空格。

ps: 当 @ 作用于普通值类型和内置简单结构体时，我们称 @ 声明的是其类型；当 @ 作用于 object和array 时，我们称 @ 声明的是对象的头信息。
...
4. 内容行行首固定2个字符，行首与内容之间至少键入一个空格（且必须是空格），以表达内容的开始。
...
1. 普通格式和科学计数法 以外的格式，必须声明类型标签（16进制、下划线等也是需要的）。
2. 特殊值也需要声明类型标签；NaN、Infinity、-Infinity的支持取决于编程语言 —— 我现在不清楚是否有语言不支持
3. int32和int64支持16进制和2进制，16进制以0x开头，2进制以0b开头 —— 必须小写x和b。
4. int32和int64支持下划线分割，不限制分割方式，但建议3个一组 —— 首尾不可以是下划线。
5. 数字如果包含了不安全字符，需要使用双引号

``` 
    {value:@i 0xFF, value1 : @i 0b10010001, value2 :@d Infinity, value3: @d NaN, value4: @i 100_000_000}
```

### bool值

1. bool 只支持 true 和 false 两个值，且大小写严格；
2. 声明类型的情况下，拼写错误将引发错误
...
1. @不可以接空白字符；
2. @label 格式后必须有一个空格，**换行不可以代替空格**。
3. 当@用于简单声明类型名时，格式为： ”@className “ , className和正式内容之间必须键入一个空格
4. 当@用于声明header结构体时，格式为："@{ ... } "
5. className为字符串类型，适用字符串配置规范。
6. header结构体不可以再内嵌header
7. header没有默认结构体，以允许用户自行扩展。
...
1. \- 和 # 总是写在行首，顶级对象之间通过空内容行分离
2. key尽量遵守一般编程语言变量命名规范，以避免引号；同时key应当保持简短，避免换行。
3. ClassName不要包含特殊字符，使得ClassName总是可以无引号表达。
4. ClassName中通过点号( . )分离，表达不同的命名空间 ，大写开头表自定义类型，基本值类型小写开头。
5. { 和 [ 与正式内容之间总是键入一个空格
6. header的定义要保持简单和简短，避免增加额外的复杂度。
...
2. 总是要加引号，key要加，value要加，手写json的时候有点难受。
3. 字符串不支持换行。
    1. 程序生成的json文件经常出现一行上千和上万个字符的情况。
    2. 手写json出现长文本时非常难受。
4. 不支持文本模式，不支持所见即所得，要处理大量的转义字符。
...
1. 支持数字使用下划线，来自TOML的提醒，我在写代码的经常用到。
