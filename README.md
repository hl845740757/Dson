...
| ^  | switch mode | 模式切换，当前行有效；<br/>纯文本模式下插入转义行；<br/>双引号模式下插入纯文本行； | - "我想插入一段正则表达式： <br/> ^ ^\[\\u4e00-\\u9fa5_a-zA-Z0-9]+$ <br/> - " |
...
4. 行首与内容之间至少键入一个空格（且必须是空格），以表达内容的开始。
5. 顶层对象必须是Object/Array/Header，你不可以直接声明一个基础值类型，顶层对象之间可以不使用逗号分隔。
6. Dson大小写敏感。
...
6. 存在一些特殊含义的字符串，用户最好总是使用双引号强调是字符串，否则可能发生兼容性问题。
...
| 引号  | 反斜杠 | 退格(BS) | 换页(FF) | 换行(LF) | 回车(CR) | 水平制表(HT) |
|-----|-----|--------|--------|--------|--------|----------|
| \\" | \\\ | \b     | \f     | \n     | \r     | \t       |
...
2. key尽量遵守一般编程语言变量命名规范，仅使用数字字母和下划线\[_a-zA-Z]，以避免引号；同时key应当保持简短，避免换行。
3. ClassName尽量不包含特殊字符，使得ClassName总是可以无引号表达。可考虑通过点号'.'或下划线'_'分隔；
4. { 和 [ 与正式内容之间总是键入一个空格
5. header的定义要保持简单和简短，避免增加额外的复杂度。
6. 字符串尽量使用无引号模式 和 段落模式输入，手写转义是麻烦易出错的工作。
7. 数字避免使用8进制
