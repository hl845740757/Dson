...
/**
 * 自定义POJO对象编解码实现接口，该接口与{@link DsonLiteCodecImpl}协同工作，为典型的桥接模式。
 * <p>
 * 实体类序列化工具类，每一个{@link DsonLiteCodec}只负责一个固定类型的解析。
 * 生成的代码会实现该接口，用户手动实现编解码时也实现该接口。
 * <br>-------------------------------什么时候手写实现？-----------------------<br>
 * 1. 一般而言，建议使用注解{@link DsonLiteSerializable}，并遵循相关规范，由注解处理器生成的类负责解析，而不是手写实现{@link DsonLiteCodec}。
 * 一旦手写实现，必须持久的进行维护。
 ...
 * date 2023/3/31
 */
public interface DsonLiteCodec<T> {
    ...
    /**
     * 该方法用于告知{@link DsonLiteCodecImpl}是否自动调用以下方法
     * {@link DsonLiteObjectWriter#writeStartObject(Object, TypeArgInfo)} ()}
     * {@link DsonLiteObjectWriter#writeEndObject()}
     * {@link DsonLiteObjectReader#readStartObject(TypeArgInfo)}
     * {@link DsonLiteObjectReader#readEndObject()}
     * <p>
     * Q：禁用该属性有什么用？
