 ...
 * <p>
 * 普通类钩子方法：
 * 1. 如果类提供了非私有的{@link DsonObjectReader}的单参构造方法，将自动调用 -- 该方法可用于final和忽略字段。
 * 2. 如果类提供了非私有的{@code readObject(DsonObjectReader)}的反序列化方法，将自动调用 -- 该方法可用于忽略字段。
 * 3. 如果类提供了非私有的{@code writeObject(DsonObjectWriter)}的序列化方法，将自动调用 -- 该方法可用于final和忽略字段。
 ...
 *
 * <h3>序列化的字段</h3>
 * 1. 默认所有字段都序列化。但如果有{@code transient}修饰或使用{@link DsonIgnore}进行注解，则不序列化。
 ...
 *
 * <h3>一些建议</h3>
 * 1. 一般而言，建议使用注解{@link DsonSerializable}，并遵循相关规范，由注解处理器生成的类负责解析，而不是实现{@link DsonCodec}。
