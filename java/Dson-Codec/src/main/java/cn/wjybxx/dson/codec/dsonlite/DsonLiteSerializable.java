 ...
 * 2. 如果是普通类，必须提供<b>非私有无参构造方法</b>，或提供非私有的{@link DsonLiteObjectReader}的单参构造方法。
 * 3. 对于普通类，所有托管给生成代码读的字段，必须提供setter或直接写权限。
 * 4. 对于普通类，所有托管给生成代码写的字段，必须提供getter或直接读权限。
 * <p>
 * 普通类钩子方法：
 * 1. 如果类提供了非私有的{@link DsonLiteObjectReader}的单参构造方法，将自动调用 -- 该方法可用于final和忽略字段。
 * 2. 如果类提供了非私有的{@code readObject(DsonLiteObjectReader)}的反序列化方法，将自动调用 -- 该方法可用于忽略字段。
 * 3. 如果类提供了非私有的{@code writeObject(DsonLiteObjectWriter)}的序列化方法，将自动调用 -- 该方法可用于final和忽略字段。
 ...
 *
 * <h3>序列化的字段</h3>
 * 1. 默认所有字段都序列化。但如果有{@code transient}修饰或使用{@link DsonLiteIgnore}进行注解，则不序列化。
 * 2. {@link DsonLiteIgnore}也可以用于将{@code transient}字段加入编解码。
 ...
 * <h3>扩展</h3>
 * Q: 是否可以不使用注解，也能序列化？
 * A: 如果不使用注解，需要手动实现{@link DsonLiteCodec}，并将其添加到注册表中。
 * （也可以加入到Scanner的扫描路径）
 *
 * <h3>一些建议</h3>
 * 1.一般而言，建议使用注解{@link DsonLiteSerializable}，并遵循相关规范，由注解处理器生成的类负责解析，而不是实现{@link DsonLiteCodec}。
...
public @interface DsonLiteSerializable {

    /**
     * 为生成的文件添加的注解
     * 比如：可以添加{@link DsonLiteCodecScanIgnore}以使得生成的代码在扫描Codec时被忽略。
     */
