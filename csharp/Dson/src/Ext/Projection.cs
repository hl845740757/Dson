...
public class Projection
{
    ...
    static Projection() {
        ...
        tempKeys.Add(KeyHeader);
        tempKeys.AddAll(ObjectKeys);
        tempKeys.AddAll(ArrayKeys);
        AllSpecialKeys = tempKeys.ToImmutableHashSet();
    }
    ...
    private class DefaultNode : Node
    {
        ...
        public DefaultNode(bool arrayLike, DsonObject<string> projectInfo) {
            ...
            projectInfo.TryGetValue(KeyHeader, out var header);
            ...
            // object 字段映射
            {
                ...
                foreach (KeyValuePair<string, DsonValue> entry in projectInfo) {
                    ...
                    if (AllSpecialKeys.Contains(key)) {
                        continue;
                    }
    ...
    private static bool IsTrue(DsonValue? dsonValue) {
        ...
        if (dsonValue.DsonType == DsonType.Boolean) {
