...
在Protobuf中，float固定4字节，double固定8字节。我也研究了一下浮点数的规范，其实可以发现：**数据集中在指数位附近**。

1. 一个不能被二进制精确表达的数，其浮点数的有效位将用尽以达到更近似的值，因此是无法压缩的。
2. 而一个能被二进制精确表达的数，有效位只使用到其精度的位置，后面就全部为0，因此是可以压缩的 -- 因此整数大多是可以压缩的。
3. 压缩算法的原理就是统计全为0的字节数，编码时不编码后导0。
4. 不过，我们并不是统计所有字节，因为数据集中在指数位附近，因此前两个字节几乎总是不为0，浮点数的前16位总是写入，因此无需统计。
5. 因此，对于Float只统计后2个字节，Double只统计后6个字节，恰好够WireType占用的比特位。
...
不过，实际上我们使用的数的分布并非如此，我们主动使用高精度浮点数的概率其实是小于低精度的数的，而且使用整数的概率也是很高的，
因此总体的收益应该是大于1/6的。