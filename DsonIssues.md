...
在Protobuf中，float固定4字节，double固定8字节。我想尝试压缩浮点数，于是研究了一下浮点数的表示法，发现一些特点：

1. **数据集中在指数位附近**，即内存的高位，这是由浮点数的求值方式决定的 * 。
2. **规格数前两个字节一定有1**，因为规格数的指数1开始，因此**前两个字节无压缩意义**。
3. 一个不能被二进制精确表达的数，其浮点数的有效位将用尽以达到更近似的值，因此是无法压缩的。
4. 而一个能被二进制精确表达的数，有效位只使用到其需要的精度的位置，后面就全部为0，因此是可以压缩的 -- 整数总是能精确表示的。

由以上的特征，可以得到压缩算法：

1. 获取浮点数的原始bits，统计后导0占用的字节数（Float只统计后2个字节，Double只统计后6个字节），在编码时跳过这些字节。
2. 在编码时，以字节为单位进行逻辑右移，将所有的非0字节编码 -- 仍然表现为小端编码。
3. 在解码时，以字节为单位进行左移，或上新读取的字节即可。

关于压缩收益  
实际的压缩收益是难以直接确定的，与我们使用的值的分布有关，但我们可以按下面这样分析：

1. 在所有的数中，能用二进制精确表达的数占1半，不能精确表达的数也占一半。
2. 对于float，共23个有效位，而前7个有效位已随着指数位序列化，因此：
    1. 2字节占比 7/23
    2. 3字节占比 8/23
    3. 4字节占比 8/23
    4. 因此平均的字节数：3.04，节省的字节数为 0.96，占比 24%
    5. 也就是说，如果我们使用的数都是可精确表达的，能节省1/4的开销；而放在全局下，能节省 1/8 的开销。

3. 对于double，共52个有效位，而前4个有效位已随着指数位序列化，因此：
    1. 2字节占比 4/52
    2. 3、4、5、6、7、8字节分别占比 8/52
    3. 因此平均的字节数：5.23，节省的字节数为 2.77，占比：34.6%
    4. 也就是说，如果我们使用的数都是可精确表达的，能节省1/3的开销；而放在全局下，能节省 1/6 的开销。
    5. 不过，实际上我们使用的数的分布并非如此，通常，我们使用小区间值的概率大于使用大区间的值，因此收益通常高于1/6。